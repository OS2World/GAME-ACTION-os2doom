  ...
  patch_t*	c[] )
{
    ...
    do
    {
	left = ((SCREENWIDTH-320)/2)+lnodes[wbs->epsd][n].x - c[i]->leftoffset;
	top = ((SCREENHEIGHT-200)/2)+lnodes[wbs->epsd][n].y - c[i]->topoffset;
...
void WI_drawDeathmatchStats(void)
{
    ...
    V_DrawPatch(((SCREENWIDTH-320)/2)+DM_TOTALSX-total->width/2,
	...
	if (playeringame[i])
	{
	    V_DrawPatch(((SCREENWIDTH-320)/2)+x-p[i]->width/2,
			((SCREENHEIGHT-200)/2)+DM_MATRIXY - WI_SPACINGY,
			FB,
			p[i]);
	    
	    V_DrawPatch(((SCREENWIDTH-320)/2)+DM_MATRIXX-p[i]->width/2,
...
void WI_drawNetgameStats(void)
{
    ...
    // draw stat titles (top line)
    V_DrawPatch(((SCREENWIDTH-320)/2)+NG_STATSX+NG_SPACINGX-kills->width,
		((SCREENHEIGHT-200)/2)+NG_STATSY, FB, kills);

    V_DrawPatch(((SCREENWIDTH-320)/2)+NG_STATSX+2*NG_SPACINGX-items->width,
		((SCREENHEIGHT-200)/2)+NG_STATSY, FB, items);

    V_DrawPatch(((SCREENWIDTH-320)/2)+NG_STATSX+3*NG_SPACINGX-secret->width,
		((SCREENHEIGHT-200)/2)+NG_STATSY, FB, secret);
    
    if (dofrags)
	V_DrawPatch(((SCREENWIDTH-320)/2)+NG_STATSX+4*NG_SPACINGX-frags->width,
		    ((SCREENHEIGHT-200)/2)+NG_STATSY, FB, frags);

    // draw stats
    y = ((SCREENHEIGHT-200)/2)+NG_STATSY + kills->height;
