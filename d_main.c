...
if ( tics < 0 )
   {
   char szMessage[80];
   sprintf( szMessage, "tics = %d", tics );
   WinMessageBox(HWND_DESKTOP,HWND_DESKTOP,szMessage,"panic", 0, MB_ICONHAND | MB_OK );
   }
           }
        while(tics <= 0);
        wipestart = nowtime;
...
// Change over to buffered output...
	...
	break;
      case commercial:
       if (plutonia == TRUE)
           sprintf (title, "                   DOOM 2: Plutonia Experiment v%i.%i                        ", DOOM_VERSION/100,DOOM_VERSION%100);
       else
       if (tnt == TRUE)
           sprintf (title, "                     DOOM 2: TNT - Evilution v%i.%i                          ", DOOM_VERSION/100,DOOM_VERSION%100);
       else
           sprintf (title, "                      DOOM 2: Hell on Earth v%i.%i                           ", DOOM_VERSION/100,DOOM_VERSION%100);
	break;
      default:
	sprintf (title,
		 "                     "
